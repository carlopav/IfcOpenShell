    ...
    def create_snap_obj(cls, obj):
        ...
        for i, snap_obj in enumerate(cls.snap_objs):
            if obj.name == snap_obj.obj.name:
                # Rebuild the cache if the object changed while a modal operator
                # is active (e.g. adding a door/window alters the wall geometry).
                # A vertex-count or world-matrix change is a cheap staleness
                # signal, so we avoid re-checking every vertex on each mouse move.
                if len(obj.data.vertices) != len(snap_obj.verts_3d) or obj.matrix_world != snap_obj.matrix_world:
                    cls.snap_objs.pop(i)
